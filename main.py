...
#iterate over the raw champion data to create a list of only champion ids and champion names
champ_keys = []*2
for key, value in raw_champion_data['data'].items():
    champ_keys.append([int(value['key']), key])

#create a dataframe from champ_keys list
...
#join champion_mastery and champion_keys dataframes to add champion names column
final_champion_mastery = pd.merge(champion_mastery, champion_keys, how='inner')

#move champion column to the first column, drop summonerId, sort by championPoints desc
